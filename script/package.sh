...
GO_LANGSERVER_VERSION="v0.1.7"

if [[ $# -eq 0 ]]; then
    echo "deploy snapshot package.."
    KIBANA_VERSION=7.4.3
...
docker run \
        ...
        /bin/bash -c "set -ex
                      ...
                      do
                        curl -OL https://github.com/elastic/go-langserver/releases/download/$GO_LANGSERVER_VERSION/go-langserver-\$PLATFORM-amd64.tar.gz
                        mkdir lib
